...
impl HiveGameState {
    ...
    pub fn pieces(&self) -> (&BTreeMap<PieceType, u32>, u8) {
        self.pieces_for_player(self.current_player)
    }

    pub fn pieces_for_player(&self, player: Player) -> (&BTreeMap<PieceType, u32>, u8) {
        match player {
...
mod test {
    ...
    fn beetle_up_blocked_test() {
        ...
        let left = zero + HexaDirection::UpLeft;
        let right = zero + HexaDirection::UpRight;
        state.place_piece(PieceType::Beetle, zero);
        state.place_piece(PieceType::Queen, up);
